...
impl<T: GridCell + Default + PartialEq> Grid<T> {
    ...
    /// Construct a grid from rows in canonical logical order.
    ///
    /// Checkpoint rows are ordered from the oldest retained history row through
    /// the last visible row.  The storage ring and its spare cache rows are an
    /// implementation detail, so restoring through this helper deliberately
    /// rebuilds the ring rather than importing its physical layout.
    #[cfg(feature = "checkpoint")]
    pub(crate) fn from_checkpoint_rows(
        lines: usize,
        columns: usize,
        max_scroll_limit: usize,
        display_offset: usize,
        ...
        if lines == 0 || columns == 0 || rows.len() < lines {
            return None;
        }

        let history_size = rows.len() - lines;
        if history_size > max_scroll_limit || display_offset > history_size {
            return None;
        }
        ...
        let mut grid = Self::new(lines, columns, max_scroll_limit);
        grid.increase_scroll_limit(history_size);
        ...
        grid.display_offset = display_offset;
        Some(grid)
    }
...
impl<T> Grid<T> {
    ...
    /// Maximum number of history rows retained by this grid.
    #[cfg(feature = "checkpoint")]
    pub(crate) fn max_scroll_limit(&self) -> usize {
        self.max_scroll_limit
    }

    /// Copy rows in stable logical order for a checkpoint.
    #[cfg(feature = "checkpoint")]
    pub(crate) fn checkpoint_rows(&self) -> Vec<Row<T>>
    where
        T: Clone,
    {
        ...
        (0..self.total_lines())
            .map(|index| {
                let line = Line(index as i32 - history_size as i32);
                Row::from_vec(self[line].into_iter().cloned().collect(), 0)
            })
            .collect()
    }
