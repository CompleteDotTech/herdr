...
/// Maximum number of combining characters retained by one cell.
///
/// This is shared by the live terminal write path and the checkpoint codec.
/// Keeping the cap at the storage boundary prevents a parser batch from
/// allocating an unbounded zero-width vector before a caller can validate the
/// resulting model.
pub const MAX_ZERO_WIDTH_CHARS: usize = 1_024;
...
impl Cell {
    ...
    #[inline]
    pub fn push_zerowidth(&mut self, character: char) -> bool {
        if self
            .extra
            .as_ref()
            .is_some_and(|extra| extra.zerowidth.len() >= MAX_ZERO_WIDTH_CHARS)
        {
            return false;
        }
...
mod tests {
    ...
    #[test]
    fn zero_width_push_is_bounded_before_growth() {
        let mut cell = Cell::default();
        for _ in 0..MAX_ZERO_WIDTH_CHARS {
            assert!(cell.push_zerowidth('\u{301}'));
        }
        assert!(!cell.push_zerowidth('\u{301}'));
        assert_eq!(cell.zerowidth().unwrap().len(), MAX_ZERO_WIDTH_CHARS);
    }
