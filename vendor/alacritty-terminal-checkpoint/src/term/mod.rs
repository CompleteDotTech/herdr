...
pub struct Term<T> {
    ...
    /// Set when a zero-width character was rejected at the live storage
    /// boundary. The parser/engine owns the policy response; keeping this
    /// marker on the model lets a handler with a void input method report
    /// the rejected write without growing the cell first.
    zero_width_overflowed: bool,
...
impl<T> Term<T> {
    ...
    /// Whether a live cell rejected a combining character at its bounded
    /// storage boundary.
    #[inline]
    pub fn zero_width_overflowed(&self) -> bool {
        self.zero_width_overflowed
    }
...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn input(&mut self, c: char) {
        ...
        if width == 0 {
            ...
            if !self.grid[line][column].push_zerowidth(c) {
                self.zero_width_overflowed = true;
            }
            return;
        }
...
mod tests {
    ...
    type CapturedColors = Arc<Mutex<Vec<(usize, Option<ansi::Rgb>)>>>;

    #[derive(Clone)]
    struct ColorCapture(CapturedColors);

    impl EventListener for ColorCapture {
        fn send_event(&self, event: Event) {
            if let Event::ColorRequest(index, color, _) = event {
                self.0.lock().unwrap().push((index, color));
            }
        }
    }

    #[test]
    fn color_query_captures_palette_before_synchronized_mutation() {
        let captured = Arc::new(Mutex::new(Vec::new()));
        let size = TermSize::new(8, 2);
        let mut term = Term::new(Config::default(), &size, ColorCapture(captured.clone()));
        let mut parser = ansi::Processor::<ansi::StdSyncHandler>::new();

        parser.advance(
            &mut term,
            b"\x1b]4;1;rgb:10/20/30\x07\x1b[?2026h\x1b]4;1;?\x07\x1b]4;1;rgb:ff/00/00\x07\x1b[?2026l",
        );
