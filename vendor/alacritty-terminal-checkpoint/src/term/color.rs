...
impl Colors {
    /// Copy the complete palette into its stable checkpoint representation.
    #[cfg(feature = "checkpoint")]
    pub(crate) fn checkpoint_values(&self) -> [Option<Rgb>; COUNT] {
        self.0
    }

    /// Rebuild a palette after the checkpoint validator has checked its length.
    #[cfg(feature = "checkpoint")]
    pub(crate) fn from_checkpoint_values(values: [Option<Rgb>; COUNT]) -> Self {
        Self(values)
    }
}
