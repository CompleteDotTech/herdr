...
pub enum Event {
    ...
    ///
    /// The palette value is captured when the query is parsed. This keeps a
    /// deferred response tied to the state at the query point, even when the
    /// event is delivered after a synchronized update has finished parsing.
    /// The attached function is a formatter which will correctly transform the
    /// RGB color into the expected escape sequence format.
    ColorRequest(usize, Option<Rgb>, Arc<dyn Fn(Rgb) -> String + Sync + Send + 'static>),
...
impl Debug for Event {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Event::ColorRequest(index, _, _) => write!(f, "ColorRequest({index})"),
