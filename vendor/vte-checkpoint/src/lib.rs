...
#[cfg(feature = "checkpoint")]
pub use checkpoint::{
    validate_serialized_size, CheckpointError, ParserCheckpointV1, ParserStateV1,
    MAX_CHECKPOINT_BYTES, MAX_CHECKPOINT_OSC_RAW, UPSTREAM_COMMIT, VTE_VERSION,
};
...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    /// Whether an OSC payload exceeded the checkpoint parser's explicit raw
    /// payload limit. An overflowed parser has discarded bytes and must be
    /// replaced by its owner instead of being checkpointed or continued.
    #[cfg(feature = "checkpoint")]
    pub fn osc_overflowed(&self) -> bool {
        self.osc_payload_overflowed()
    }

    #[inline]
    fn osc_payload_overflowed(&self) -> bool {
        #[cfg(feature = "checkpoint")]
        {
            self.osc_overflowed
        }
    ...
    fn action_osc_put(&mut self, byte: u8) {
        ...
        #[cfg(all(feature = "std", feature = "checkpoint"))]
        if self.osc_raw.len() >= crate::checkpoint::MAX_CHECKPOINT_OSC_RAW {
    ...
    fn osc_end<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        if !self.osc_payload_overflowed() {
            self.action_osc_put_param();
...
mod tests {
    ...
    fn exceed_max_buffer_size() {
        #[cfg(feature = "checkpoint")]
        const NUM_BYTES: usize = crate::checkpoint::MAX_CHECKPOINT_OSC_RAW + 100;
        #[cfg(not(feature = "checkpoint"))]
        ...
        #[cfg(feature = "checkpoint")]
        {
            ...
            assert!(!dispatcher
                .dispatched
                .iter()
                .any(|event| matches!(event, Sequence::Osc(_, _))));
        }
