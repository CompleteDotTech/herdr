...
/// Versioned checkpoints for [`Processor`] and its synchronized-update
/// timeout. The module is part of the vte fork so it can read the processor's
/// private parser and buffering fields without exposing layout assumptions to
/// callers.
...
pub mod checkpoint {
    ...
    use super::{
        Processor, ProcessorState, StdSyncHandler, SyncState, Timeout, SYNC_BUFFER_SIZE,
        SYNC_UPDATE_TIMEOUT,
    };
    pub use crate::checkpoint::{
        validate_serialized_size, CheckpointError, ParserCheckpointV1, UPSTREAM_COMMIT, VTE_VERSION,
    };

    /// The persisted representation of a synchronized-update timeout.
    ///
    /// Only relative time is serialized. A restored timeout is re-armed from
    /// the restore instant, so platform-specific `Instant` bytes never cross
    /// the persistence boundary.
    #[derive(Clone, Copy, Debug, Default, Eq, PartialEq, Serialize, Deserialize)]
    #[serde(deny_unknown_fields)]
    pub struct SyncTimeoutCheckpointV1 {
        pub pending: bool,
        pub remaining_ms: u16,
    }

    impl SyncTimeoutCheckpointV1 {
        pub const VERSION: u16 = 1;
        pub const MAX_REMAINING_MS: u16 = 150;

        pub fn validate(&self) -> Result<(), CheckpointError> {
            if self.remaining_ms > Self::MAX_REMAINING_MS {
                return Err(CheckpointError::LimitExceeded {
                    field: "sync_timeout.remaining_ms",
                    actual: usize::from(self.remaining_ms),
                    maximum: usize::from(Self::MAX_REMAINING_MS),
                });
            }
            if !self.pending && self.remaining_ms != 0 {
                return Err(CheckpointError::Invalid("inactive sync timeout has remaining time"));
            }
            Ok(())
        }
    }

    /// Complete processor continuation state, including bytes held by a
    /// synchronized update and the REP preceding character.
    #[derive(Clone, Debug, Eq, PartialEq, Serialize)]
    pub struct ProcessorCheckpointV1 {
        pub schema: String,
        pub version: u16,
        pub parser: ParserCheckpointV1,
        pub preceding_char: Option<char>,
        pub synchronized_bytes: Vec<u8>,
        pub sync_timeout: SyncTimeoutCheckpointV1,
    }

    #[derive(Deserialize)]
    #[serde(deny_unknown_fields)]
    struct ProcessorCheckpointV1Fields {
        schema: String,
        version: u16,
        parser: ParserCheckpointV1,
        preceding_char: Option<char>,
        #[serde(deserialize_with = "deserialize_sync_bytes")]
        synchronized_bytes: Vec<u8>,
        sync_timeout: SyncTimeoutCheckpointV1,
    }

    fn deserialize_sync_bytes<'de, D>(deserializer: D) -> Result<Vec<u8>, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        crate::checkpoint::deserialize_bounded_vec(
            deserializer,
            ProcessorCheckpointV1::MAX_SYNC_BYTES,
        )
    }

    impl<'de> Deserialize<'de> for ProcessorCheckpointV1 {
        fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
        where
            D: serde::Deserializer<'de>,
        {
            let fields = ProcessorCheckpointV1Fields::deserialize(deserializer)?;
            Ok(Self {
                schema: fields.schema,
                version: fields.version,
                parser: fields.parser,
                preceding_char: fields.preceding_char,
                synchronized_bytes: fields.synchronized_bytes,
                sync_timeout: fields.sync_timeout,
            })
        }
    }

    impl Default for ProcessorCheckpointV1 {
        fn default() -> Self {
            Self {
                schema: Self::SCHEMA.to_owned(),
                version: Self::VERSION,
                parser: ParserCheckpointV1::default(),
                preceding_char: None,
                synchronized_bytes: Vec::new(),
                sync_timeout: SyncTimeoutCheckpointV1::default(),
            }
        }
    }

    impl ProcessorCheckpointV1 {
        pub const SCHEMA: &'static str = "herdr.vte.processor";
        pub const VERSION: u16 = 1;
        pub const MAX_SYNC_BYTES: usize = SYNC_BUFFER_SIZE;

        pub fn validate(&self) -> Result<(), CheckpointError> {
            if self.schema != Self::SCHEMA {
                return Err(CheckpointError::Invalid("processor schema"));
            }
            if self.version != Self::VERSION {
                return Err(CheckpointError::Invalid("processor version"));
            }
            self.parser.validate()?;
            if self.synchronized_bytes.len() > Self::MAX_SYNC_BYTES {
                return Err(CheckpointError::LimitExceeded {
                    field: "synchronized_bytes",
                    actual: self.synchronized_bytes.len(),
                    maximum: Self::MAX_SYNC_BYTES,
                });
            }
            self.sync_timeout.validate()?;
            if !self.sync_timeout.pending && !self.synchronized_bytes.is_empty() {
                return Err(CheckpointError::Invalid(
                    "synchronized bytes without an active timeout",
                ));
            }
            Ok(())
        }
    ...
    /// A timeout implementation whose checkpoint is independent of platform
    /// clock representation.
    #[derive(Debug, Default)]
    pub struct CheckpointSyncTimeout {
        deadline: Option<Instant>,
    }

    impl CheckpointSyncTimeout {
        pub fn checkpoint(&self) -> SyncTimeoutCheckpointV1 {
            let deadline = match self.deadline {
                Some(deadline) => deadline,
                None => return SyncTimeoutCheckpointV1::default(),
            };
            let remaining = deadline
                .saturating_duration_since(Instant::now())
                .as_millis()
                .min(u128::from(SyncTimeoutCheckpointV1::MAX_REMAINING_MS));
            SyncTimeoutCheckpointV1 { pending: true, remaining_ms: remaining as u16 }
        }

        pub fn from_checkpoint(
            checkpoint: &SyncTimeoutCheckpointV1,
        ) -> Result<Self, CheckpointError> {
            checkpoint.validate()?;
            let deadline = checkpoint.pending.then(|| {
                Instant::now() + Duration::from_millis(u64::from(checkpoint.remaining_ms))
            });
            Ok(Self { deadline })
        }

        /// Whether the persisted relative deadline has elapsed.
        pub fn expired(&self) -> bool {
            self.deadline.map_or(false, |deadline| Instant::now() >= deadline)
        }
    }

    impl Timeout for CheckpointSyncTimeout {
        fn set_timeout(&mut self, duration: Duration) {
            let duration = duration.min(SYNC_UPDATE_TIMEOUT);
    ...
    /// Extension required by the generic processor checkpoint methods.
    pub trait CheckpointTimeout: Timeout {
        fn checkpoint(&self) -> SyncTimeoutCheckpointV1;
        fn from_checkpoint(checkpoint: &SyncTimeoutCheckpointV1) -> Result<Self, CheckpointError>
        where
            Self: Sized;
        fn expired(&self) -> bool;
    }

    impl CheckpointTimeout for CheckpointSyncTimeout {
        fn checkpoint(&self) -> SyncTimeoutCheckpointV1 {
    ...
    impl StdSyncHandler {
        /// Export the stock timeout using a relative duration.
        pub fn checkpoint(&self) -> SyncTimeoutCheckpointV1 {
        ...
        pub fn from_checkpoint(
            ...
            checkpoint.validate()?;
            let timeout = checkpoint.pending.then(|| {
                Instant::now() + Duration::from_millis(u64::from(checkpoint.remaining_ms))
            });
            Ok(Self { timeout })
        }
        ...
        pub fn expired(&self) -> bool {
            self.timeout.map_or(false, |deadline| Instant::now() >= deadline)
        }
    }

    impl CheckpointTimeout for StdSyncHandler {
        fn checkpoint(&self) -> SyncTimeoutCheckpointV1 {
            Self::checkpoint(self)
        }
    ...
    where
        T: CheckpointTimeout,
    {
        /// Export parser, preceding character, synchronized bytes, and
        /// clock-relative timeout state.
        ///
        /// # Panics
        ///
        /// Panics if the parser discarded bytes from an oversized OSC
        /// payload. Use [`Self::try_checkpoint`] when the input source is not
        /// already known to stay within the checkpoint limit.
        pub fn checkpoint(&self) -> ProcessorCheckpointV1 {
            ProcessorCheckpointV1 {
                schema: ProcessorCheckpointV1::SCHEMA.to_owned(),
                version: ProcessorCheckpointV1::VERSION,
                parser: self.parser.checkpoint(),
                preceding_char: self.state.preceding_char,
                synchronized_bytes: self.state.sync_state.buffer.clone(),
                sync_timeout: self.state.sync_state.timeout.checkpoint(),
            }
        }

        /// Export only when all current fields satisfy fork limits.
        pub fn try_checkpoint(&self) -> Result<ProcessorCheckpointV1, CheckpointError> {
            self.parser.try_checkpoint()?;
            if self.state.sync_state.buffer.len() > ProcessorCheckpointV1::MAX_SYNC_BYTES {
                return Err(CheckpointError::LimitExceeded {
                    field: "synchronized_bytes",
                    actual: self.state.sync_state.buffer.len(),
                    maximum: ProcessorCheckpointV1::MAX_SYNC_BYTES,
                });
            }
        ...
        /// Restore without replaying the prefix or installing unchecked
        /// parser/buffer state.
        pub fn from_checkpoint(
            checkpoint: &ProcessorCheckpointV1,
        ) -> Result<Self, CheckpointError> {
            checkpoint.validate()?;
            let timeout = T::from_checkpoint(&checkpoint.sync_timeout)?;
            Self::from_checkpoint_with_timeout(checkpoint, timeout)
        }

        /// Restore with a caller-supplied timeout implementation after
        /// validating all serialized parser and buffer state. This is useful
        /// for an owner that supplies a clock-aware timeout wrapper while
        /// retaining the checkpoint's relative timeout policy separately.
        pub fn from_checkpoint_with_timeout(
            checkpoint: &ProcessorCheckpointV1,
            timeout: T,
        ) -> Result<Self, CheckpointError> {
            checkpoint.validate()?;
            if timeout.pending_timeout() != checkpoint.sync_timeout.pending {
                return Err(CheckpointError::Invalid(
                    "supplied timeout pending state disagrees with checkpoint",
                ));
            }
            let parser = crate::Parser::from_checkpoint(&checkpoint.parser)?;
            let mut buffer = Vec::with_capacity(checkpoint.synchronized_bytes.len());
            buffer.extend_from_slice(&checkpoint.synchronized_bytes);
            Ok(Self {
                state: ProcessorState {
                    preceding_char: checkpoint.preceding_char,
                    sync_state: SyncState { timeout, buffer },
                },
                parser,
            })
        }

        /// Validate and atomically replace this processor.
        ...
        /// The owner calls this before advancing/rendering when the relative
        /// synchronized-update deadline has elapsed.
        pub fn sync_timeout_expired(&self) -> bool {
            self.state.sync_state.timeout.expired()
        }

        /// Whether the parser discarded bytes from an oversized OSC payload.
        /// The owner must replace this processor before accepting more input.
        pub fn osc_overflowed(&self) -> bool {
            self.parser.osc_overflowed()
        }
    }
}
...
#[cfg(feature = "checkpoint")]
pub use checkpoint::{
    CheckpointSyncTimeout, CheckpointTimeout, ProcessorCheckpointV1, SyncTimeoutCheckpointV1,
};
