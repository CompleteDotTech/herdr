...
use crate::{
    ...
    transport::{PeerIdentity, TerminalConnection, TransportResponse, MAX_RESPONSE_BODY_BYTES},
    DaemonEndpoint,
};
...
pub(crate) fn request(
    ...
    Ok(TransportResponse {
        response: read_response(&mut stream, deadline, max_response_body_bytes)?,
        peer_identity,
    })
}

#[cfg(windows)]
pub(crate) fn open_terminal_connection(
    endpoint: &DaemonEndpoint,
    path: &str,
    body: &[u8],
    expected_peer: &PeerIdentity,
) -> Result<TerminalConnection, ClientError> {
    open_terminal_connection_with_headers(endpoint, path, body, expected_peer, "")
}

#[cfg(windows)]
pub(crate) fn open_terminal_checkpoint_connection(
    endpoint: &DaemonEndpoint,
    path: &str,
    body: &[u8],
    expected_peer: &PeerIdentity,
) -> Result<TerminalConnection, ClientError> {
    open_terminal_connection_with_headers(
        endpoint,
        path,
        body,
        expected_peer,
        crate::transport::TERMINAL_CHECKPOINT_REQUEST_HEADERS,
    )
}

#[cfg(windows)]
fn open_terminal_connection_with_headers(
    endpoint: &DaemonEndpoint,
    path: &str,
    body: &[u8],
    expected_peer: &PeerIdentity,
    ...
    const OPERATION: &str = "failed to write Coven terminal request";
    write_windows_pipe_with_deadline(&mut stream, request.as_bytes(), deadline, OPERATION)?;
    write_windows_pipe_with_deadline(&mut stream, body, deadline, OPERATION)?;
    let framed = read_framed_response_with_remainder(
        &mut WindowsPipeReader(&stream),
        deadline,
        crate::terminal::MAX_TERMINAL_ATTACH_BODY_BYTES,
    )?;
    Ok(TerminalConnection {
        response: framed.response,
        response_headers: framed.response_headers,
        stream: Box::new(WindowsTerminalStream(stream)),
...
fn read_response(
    ...
    read_windows_framed_response(stream, deadline, max_body_bytes)
}
...
#[cfg(windows)]
struct WindowsTerminalStream(std::fs::File);

#[cfg(windows)]
impl Read for WindowsTerminalStream {
    fn read(&mut self, buffer: &mut [u8]) -> Result<usize, std::io::Error> {
        WindowsPipeReader(&self.0).read(buffer)
    }
}

#[cfg(windows)]
impl Write for WindowsTerminalStream {
...
#[cfg(windows)]
fn read_framed_response_with_remainder<R: Read>(
    reader: &mut R,
    deadline: Instant,
    max_body_bytes: usize,
) -> Result<WindowsFramedResponse, ClientError> {
    ...
    loop {
        if let Some((head, body)) = split_response(&response) {
            ...
            if body.len() >= content_length {
                ...
                let buffered_remainder = body.split_off(content_length);
                return Ok(WindowsFramedResponse {
                    response: HttpResponse { status, body },
                    response_headers: head.to_vec(),
                    buffered_remainder,
                });
            }
...
mod tests {
    ...
    #[test]
    fn response_reader_rejects_declared_route_oversize_before_body_read() {
        let max_body_bytes = crate::terminal_control::MAX_TERMINAL_CONTROL_RESPONSE_BYTES;
        let header = format!(
            "HTTP/1.1 200 OK\r\nContent-Length: {}\r\n\r\n",
            max_body_bytes + 1
        )
        ...
        let error = match read_framed_response(
            ...
            Ok(_) => panic!("a declared terminal-control response over the route cap must fail"),
            Err(error) => error,
        };

        assert!(matches!(
            error,
            ClientError::ResponseTooLarge { max_bytes } if max_bytes == max_body_bytes
        ));
        assert_eq!(
            reader.reads, 1,
            "the body must not be read after Content-Length"
        );
    }

    #[test]
    fn response_reader_rejects_chunked_encoding_before_body_read() {
        let mut reader = HeaderOnlyReader {
            header: Cursor::new(b"HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\n".to_vec()),
            reads: 0,
        };
        ...
        let error = match read_framed_response(
            ...
            Ok(_) => panic!("chunked terminal-control responses must be rejected"),
            Err(error) => error,
        };
        ...
        assert!(matches!(
            ...
            ClientError::InvalidHttpResponse(message)
                if message.contains("unsupported Transfer-Encoding")
        ));
        assert_eq!(reader.reads, 1, "chunked body bytes must not be read");
    }
