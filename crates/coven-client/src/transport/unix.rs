...
use crate::{
    ...
    transport::{
        write_with_deadline, HttpResponse, PeerIdentity, TerminalConnection, TransportResponse,
    },
    ClientError, DaemonEndpoint,
};

#[cfg(test)]
use crate::transport::MAX_RESPONSE_BODY_BYTES;
...
pub(crate) fn open_terminal_connection(
    ...
    open_terminal_connection_with_headers(endpoint, method_path, body, expected_peer, "")
}
...
fn open_terminal_connection_with_headers(
    ...
    let pending = request_retaining_connection_bound(
        ...
        RetainedRequestOptions {
            ...
            max_response_body_bytes: crate::terminal::MAX_TERMINAL_ATTACH_BODY_BYTES,
            bind_process_identity: false,
            shutdown_write: false,
            extra_headers,
        },
    )?;
    let AuthenticatedUnixResponse {
        response,
        peer_identity: _,
        stream,
        response_headers,
...
fn request_retaining_connection_bound(
    ...
    let request = format!(
        ...
        body.len(), options.extra_headers
    );
...
fn read_framed_response<R: Read>(
    ...
    Ok(FramedResponse {
        ...
        response_headers: received[..body_start].to_vec(),
        buffered_remainder,
    })
}
...
mod tests {
    ...
    fn response_reader_rejects_declared_route_oversize_before_body_read() {
        ...
        assert!(matches!(
            error,
            crate::ClientError::ResponseTooLarge { max_bytes } if max_bytes == max_body_bytes
        ));
        assert_eq!(
            reader.reads, 1,
    ...
    fn response_reader_rejects_chunked_encoding_before_body_read() {
        ...
        assert!(matches!(
            error,
            crate::ClientError::InvalidHttpResponse(message)
