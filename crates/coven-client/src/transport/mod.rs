...
/// Request headers used when the attach route switches from its bounded JSON
/// handshake to the CTS2 stream. The body remains the authoritative typed
/// negotiation; these headers make accidental raw/HTTP response reuse fail
/// closed at the client boundary.
pub(crate) const TERMINAL_CHECKPOINT_REQUEST_HEADERS: &str =
    "Accept: application/coven-terminal-handshake+json\r\n\
Connection: Upgrade\r\n\
Upgrade: coven-terminal-cts2\r\n\
...
/// A connection whose HTTP negotiation has completed and whose remaining
/// bytes belong to the terminal frame stream. The platform modules perform
/// the same endpoint and peer validation as ordinary requests before creating
/// this value.
...
pub(crate) struct TerminalConnection {
    pub(crate) response: HttpResponse,
    /// The bounded HTTP header block that preceded the handshake body.  A
    /// long-lived attachment must validate the protocol upgrade marker before
    /// it interprets any following bytes as CTS2 frames.
    pub(crate) response_headers: Vec<u8>,
    pub(crate) stream: Box<dyn ReadWrite + Send>,
    pub(crate) buffered_remainder: Vec<u8>,
}
...
pub(crate) fn request(
    ...
    request_with_peer(endpoint, method, path, body, None, MAX_RESPONSE_BODY_BYTES)
}
...
pub(crate) fn open_terminal(
    ...
    if path != "/api/v1/terminal/attach" {
        return Err(ClientError::InvalidHttpResponse(
            "terminal attachment path is invalid".to_owned(),
        ));
    }
    ...
    #[cfg(unix)]
    {
        unix::open_terminal_connection(endpoint, path, body, expected_peer)
    }

    #[cfg(windows)]
    {
        windows::open_terminal_connection(endpoint, path, body, expected_peer)
    }

    #[cfg(not(any(unix, windows)))]
    {
        let _ = (endpoint, path, body, expected_peer);
        Err(ClientError::UnsupportedPlatform)
    }
}

/// Open the same owner-local attach route while requesting the checkpoint
/// stream upgrade.  The platform transports keep the authenticated peer
/// checks and HTTP body bound used by the legacy RawBytesV1 attachment.
pub(crate) fn open_terminal_checkpoint(
    endpoint: &DaemonEndpoint,
    path: &str,
    body: &[u8],
    expected_peer: &PeerIdentity,
    ...
    if body.len() > crate::terminal_checkpoint::MAX_TERMINAL_CHECKPOINT_ATTACH_BODY_BYTES {
        return Err(ClientError::RequestTooLarge {
            max_bytes: crate::terminal_checkpoint::MAX_TERMINAL_CHECKPOINT_ATTACH_BODY_BYTES,
            actual_bytes: body.len(),
        });
    }
    ...
    #[cfg(unix)]
    {
        unix::open_terminal_checkpoint_connection(endpoint, path, body, expected_peer)
    }

    #[cfg(windows)]
    {
        windows::open_terminal_checkpoint_connection(endpoint, path, body, expected_peer)
    }
