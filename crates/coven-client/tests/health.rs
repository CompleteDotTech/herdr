...
#[test]
fn checkpoint_request_has_a_complete_header_boundary_without_half_closing() {
    use coven_client::terminal_checkpoint::TerminalCheckpointAttachRequest;
    use std::{
    ...
    let server = std::thread::spawn(move || {
        ...
        // Hang guards only; no latency assertion depends on these durations.
        health
            .set_read_timeout(Some(Duration::from_secs(5)))
            .unwrap();
        let mut request = String::new();
        health.read_to_string(&mut request).unwrap();
        ...
        loop {
            ...
            assert!(headers.len() + line.len() <= 64 * 1024);
            headers.push_str(&line);
            if line == "\r\n" {
                break;
            }
        }
        assert!(headers.starts_with("POST /api/v1/terminal/attach HTTP/1.1\r\n"));
        assert!(headers.contains("X-Coven-Terminal-Codec: checkpointV1\r\n\r\n"));
        ...
        let request: TerminalCheckpointAttachRequest = serde_json::from_slice(&body).unwrap();
        assert_eq!(request.protocol_version, 2);
        assert_eq!(request.session_id, "fixture-session");
        // A complete request is available while the write side remains open;
        // waiting for EOF here would make real streaming attachment deadlock.
        let body = r#"{"ok":false,"error":{"code":"checkpoint_protocol_unavailable","message":"fixture has no terminal"}}"#;
        write!(
            stream,
    ...
    let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
    let result = client.attach_terminal_checkpoint(TerminalCheckpointAttachRequest::new(
        "fixture-session",
        "00000000-0000-0000-0000-000000000041",
    ));
    assert!(
        matches!(result, Err(ClientError::Daemon { status: 409, ref error })
        if error.code == "checkpoint_protocol_unavailable")
    );
    server.join().unwrap();
}
...
fn health_with_capabilities(capabilities: serde_json::Value) -> String {
    ...
    health["executionAuthority"] = serde_json::json!({"id":"authority-fixture","generation":1});
    health.to_string()
}
...
fn execution_fixture_request() -> coven_client::execution::ExecutionRequest {
    serde_json::from_value(serde_json::json!({
        "contract": coven_client::execution::CONTRACT,
        "requestId": "request-1",
        "scope": {"projectId":"project-1", "profileId":"fixture", "policyGeneration":1},
        "authority":{"id":"authority-fixture","generation":1},
        "expiresAt": 2_000_000_000,
        "intent": {"kind":"launch", "harness":"codex", "mode":"transcript", "model":null, "cwd":".", "prompt":"synthetic test"}
    })).unwrap()
}

fn execution_fixture_outcome(request: &coven_client::execution::ExecutionRequest) -> String {
    serde_json::json!({
        "contract": coven_client::execution::CONTRACT,
        "requestId": request.request_id,
        "scope": request.scope,
        "payloadDigest": request.digest().unwrap(),
        "sessionId":"session-1", "generation":1,
        "state":"unresolved", "delivery":"unknown", "sessionStatus":null, "exitCode":null
    })
    .to_string()
}

#[test]
fn execution_request_requires_exact_contract_without_disabling_legacy_reads() {
    let home = TestHome::new();
    let health = health_with_capabilities(serde_json::json!({
        "sessions":true, "structuredErrors":true,
        "executionRequestContracts":["coven.execution-request.v1"]
    }));
    let server = serve_responses(
        &home.path,
        vec![
            ("/api/v1/health".into(), 200, health),
            ("<peer-check>".into(), 0, String::new()),
            ("/api/v1/sessions".into(), 200, "[]".into()),
        ],
    );
    let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
    let error = client
        .execution_request(&execution_fixture_request())
        .unwrap_err();
    assert!(matches!(
        error,
        ClientError::CapabilityUnavailable {
            capability: "executionRequestContracts"
        }
    ));
    let sessions: serde_json::Value = client.get_json(unpaginated_sessions_read()).unwrap();
    assert_eq!(sessions, serde_json::json!([]));
    server.join().unwrap();
}

#[test]
fn execution_peer_replacement_sends_no_mutation_and_lookup_reconciles_unknown() {
    let home = TestHome::new();
    let health = health_with_capabilities(serde_json::json!({
        "structuredErrors":true,
        "executionRequestContracts":[coven_client::execution::CONTRACT],
        "executionRequestOperations":["launch_transcript"]
    }));
    let old = serve_once(&home.path, 200, health.clone());
    let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
    client.health().unwrap();
    old.join().unwrap();
    // Retain the old inode so replacement cannot reuse the same filesystem identity.
    fs::rename(
        home.path.join("coven.sock"),
        home.path.join("old-coven.sock"),
    )
    .unwrap();
    let request = execution_fixture_request();
    let replacement = serve_responses(
        ...
        vec![
            ...
            ("/api/v1/health".into(), 200, health),
            (
                "/api/v1/execution-requests/project-1/fixture/request-1".into(),
                200,
                execution_fixture_outcome(&request),
            ),
        ],
    );
    assert!(matches!(
        client.execution_request(&request),
        Err(ClientError::DaemonInstanceChanged)
    ));
    let outcome = client
    ...
    assert_eq!(
        outcome.state,
        coven_client::execution::RequestState::Unresolved
    );
    assert_eq!(
        outcome.delivery,
        coven_client::execution::DeliveryState::Unknown
    );
    replacement.join().unwrap();
}

#[test]
fn execution_response_for_different_scope_is_not_success() {
    let home = TestHome::new();
    let request = execution_fixture_request();
    let mut outcome: serde_json::Value =
        serde_json::from_str(&execution_fixture_outcome(&request)).unwrap();
    outcome["scope"]["projectId"] = serde_json::json!("different-project");
    let health = health_with_capabilities(serde_json::json!({
        "structuredErrors":true,
        "executionRequestContracts":[coven_client::execution::CONTRACT],
        "executionRequestOperations":["launch_transcript"]
    ...
    let server = serve_responses(
        ...
        vec![
            ("/api/v1/health".into(), 200, health),
            (
                "/api/v1/execution-requests".into(),
                200,
                outcome.to_string(),
            ),
        ],
    ...
    let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
    let error = client.execution_request(&request).unwrap_err();
    assert!(matches!(error, ClientError::InvalidExecutionResponse(_)));
    assert!(!error.request_was_definitely_not_sent());
    server.join().unwrap();
}

#[test]
fn execution_operation_missing_sends_no_request_body() {
    let home = TestHome::new();
    let health = health_with_capabilities(serde_json::json!({
        "structuredErrors":true,
        "executionRequestContracts":[coven_client::execution::CONTRACT]
    ...
    let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
    assert!(matches!(
        client.execution_request(&execution_fixture_request()),
        Err(ClientError::CapabilityUnavailable {
            capability: "executionRequestOperations"
        })
    ));
    server.join().unwrap();
}
...
#[test]
fn execution_lookup_rejects_another_payload_under_same_identity() {
    let home = TestHome::new();
    let request = execution_fixture_request();
    let mut outcome: serde_json::Value =
        serde_json::from_str(&execution_fixture_outcome(&request)).unwrap();
    outcome["payloadDigest"] = serde_json::json!("0".repeat(64));
    let health = health_with_capabilities(serde_json::json!({
        "structuredErrors":true,
        "executionRequestContracts":[coven_client::execution::CONTRACT]
    }));
    ...
    let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
    assert!(matches!(
        client.lookup_execution_request(
            &request.scope,
            &request.request_id,
            &request.digest().unwrap(),
            &request.intent.target(),
        ),
        Err(ClientError::InvalidExecutionResponse(_))
    ));
    server.join().unwrap();
}

#[test]
fn execution_controls_bind_response_session_and_generation() {
    use coven_client::execution::{ExecutionIntent, ExecutionSessionId};
    for lookup in [false, true] {
        for wrong_generation in [false, true] {
            let home = TestHome::new();
            let mut request = execution_fixture_request();
            request.intent = ExecutionIntent::Stop {
                session_id: ExecutionSessionId::new("expected-session").unwrap(),
                generation: 7,
            };
            let mut outcome: serde_json::Value =
                serde_json::from_str(&execution_fixture_outcome(&request)).unwrap();
            outcome["sessionId"] = serde_json::json!(if wrong_generation {
                "expected-session"
            } else {
                "wrong-session"
            });
            outcome["generation"] = serde_json::json!(if wrong_generation { 8 } else { 7 });
            let health = health_with_capabilities(serde_json::json!({
                "structuredErrors":true,"executionRequestContracts":[coven_client::execution::CONTRACT],"executionRequestOperations":["stop"]
            }));
            let path = if lookup {
                "/api/v1/execution-requests/project-1/fixture/request-1"
            } else {
            ...
            let server = serve_responses(
                ...
                vec![
                    ("/api/v1/health".into(), 200, health),
                    (path.into(), 200, outcome.to_string()),
                ],
            );
            let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
...
fn source_fixture_read() -> coven_client::source::SourceRead {
    serde_json::from_value(serde_json::json!({
        "contract":coven_client::source::CONTRACT,"hostId":"host",
        "scope":{"projectId":"project","profileId":"profile","policyGeneration":1},
        "sessionId":"opaque/session%id","generation":1,"cursor":null,"limit":2
    }))
    .unwrap()
}

#[test]
fn source_read_requires_exact_capability_before_sending_query() {
    let home = TestHome::new();
    let server = serve_responses(
        &home.path,
        vec![
            (
                "/api/v1/health".into(),
                200,
                health_with_capabilities(
                    serde_json::json!({"structuredErrors":true,"executionSourceContracts":["future"]}),
                ),
            ),
            ("<peer-check>".into(), 0, String::new()),
        ],
    ...
    let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
    assert!(matches!(
        client.read_execution_source(&source_fixture_read()),
        Err(ClientError::CapabilityUnavailable {
            capability: "executionSourceContracts"
        })
    ));
    server.join().unwrap();
}
...
#[test]
fn source_read_uses_fixed_route_and_rejects_other_bindings() {
    for field in ["valid", "host", "profile", "session", "generation"] {
        let home = TestHome::new();
        let request = source_fixture_read();
        let mut reply = serde_json::json!({
            "contract":coven_client::source::CONTRACT,
            "source":{"hostId":request.host_id,"profileId":request.scope.profile_id,"ledgerId":"ledger","epoch":1},
            "scope":request.scope,"sessionId":request.session_id,"generation":1,
            "health":{"daemonLive":true,"writerState":"unknown","writerQueuedBytes":null,"writerDroppedOutputBytes":null},
            "result":{"kind":"snapshot","cursor":{"ledgerId":"ledger","epoch":1,"afterSeq":0,"revision":1},"projection":{"lifecycle":"running","exitCode":null,"archived":false,"droppedOutputBytes":null}}
        });
        match field {
            "host" => reply["source"]["hostId"] = serde_json::json!("other"),
            "profile" => reply["source"]["profileId"] = serde_json::json!("other"),
            "session" => reply["sessionId"] = serde_json::json!("other"),
            "generation" => reply["generation"] = serde_json::json!(2),
            _ => {}
        }
        let server = serve_responses(
            &home.path,
            vec![
                (
                    ...
                    200,
                    health_with_capabilities(
                        serde_json::json!({"structuredErrors":true,"executionSourceContracts":[coven_client::source::CONTRACT]}),
                    ),
                ),
                (
                    "/api/v1/execution-source/read".into(),
                    200,
                    reply.to_string(),
                ),
            ],
        ...
        let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
        let result = client.read_execution_source(&request);
        if field == "valid" {
            result.unwrap();
        } else {
            assert!(matches!(result, Err(ClientError::InvalidSourceResponse(_))));
        }
        server.join().unwrap();
    }
}
...
#[test]
fn execution_authority_mismatch_sends_no_mutation_body() {
    let home = TestHome::new();
    let health = health_with_capabilities(serde_json::json!({
        "structuredErrors":true,"executionRequestContracts":[coven_client::execution::CONTRACT],
        "executionRequestOperations":["launch_transcript"]
    }));
    let server = serve_responses(&home.path, vec![("/api/v1/health".into(), 200, health)]);
    let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
    let mut request = execution_fixture_request();
    request.authority.id = coven_client::execution::AuthorityId::new("stale-authority").unwrap();
    assert!(matches!(
        client.execution_request(&request),
        Err(ClientError::InvalidExecutionRequest(_))
    ));
    server.join().unwrap();
}

fn transcript_fixture_read() -> coven_client::transcript::TranscriptRead {
    serde_json::from_value(serde_json::json!({
        "contract":coven_client::transcript::CONTRACT,"hostId":"host",
        "scope":{"projectId":"project","profileId":"profile","policyGeneration":1},
        "authority":{"id":"authority-fixture","generation":1},
        "sessionId":"opaque/session%id","generation":1,"cursor":null,"limit":2
    }))
...
#[test]
fn transcript_read_negotiates_exact_contract_before_sending_text_request() {
    let home = TestHome::new();
    let server = serve_responses(
        &home.path,
        vec![
            (
                "/api/v1/health".into(),
                200,
                health_with_capabilities(serde_json::json!({
                    "structuredErrors":true,"executionTranscriptContracts":["future"]
                })),
            ),
            ("<peer-check>".into(), 0, String::new()),
        ],
    ...
    let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
    assert!(matches!(
        client.read_execution_transcript(&transcript_fixture_read()),
        Err(ClientError::CapabilityUnavailable {
            capability: "executionTranscriptContracts"
        })
    ));
    server.join().unwrap();
}
...
#[test]
fn transcript_read_rejects_stale_authority_before_body() {
    let home = TestHome::new();
    let server = serve_responses(
        &home.path,
        vec![(
            ...
            200,
            health_with_capabilities(serde_json::json!({
                "structuredErrors":true,"executionTranscriptContracts":[coven_client::transcript::CONTRACT]
            })),
        )],
    );
    let mut request = transcript_fixture_read();
    request.authority.generation += 1;
    let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
    assert!(matches!(
        client.read_execution_transcript(&request),
        Err(ClientError::InvalidTranscriptRequest(_))
    ));
    server.join().unwrap();
}

#[test]
fn transcript_read_uses_bound_route_and_rejects_substituted_reply() {
    for field in ["valid", "host", "session", "authority", "generation"] {
        let home = TestHome::new();
        let request = transcript_fixture_read();
        let mut reply = serde_json::json!({
            "contract":coven_client::transcript::CONTRACT,
            "source":{"hostId":"host","profileId":"profile","ledgerId":"ledger","epoch":1},
            "scope":request.scope,"authority":request.authority,"sessionId":request.session_id,"generation":1,
            "health":{"daemonLive":true,"writerState":"healthy","writerQueuedBytes":0,"writerDroppedOutputBytes":0},
            "projection":{"lifecycle":"running","exitCode":null,"archived":false,"droppedOutputBytes":0},
            "result":{"kind":"snapshot","cursor":{"ledgerId":"ledger","epoch":1,"afterSeq":2,"revision":1},"prefixOmitted":false,"chunks":[{"seq":2,"content":{"kind":"text","data":"fixture output\n","omittedPrefixBytes":0}}]}
        });
        match field {
            "host" => reply["source"]["hostId"] = serde_json::json!("other"),
            "session" => reply["sessionId"] = serde_json::json!("other"),
            "authority" => reply["authority"]["generation"] = serde_json::json!(2),
            "generation" => reply["generation"] = serde_json::json!(2),
            _ => {}
        }
        let server = serve_responses(
            ...
            vec![
                (
                    ...
                    200,
                    health_with_capabilities(
                        serde_json::json!({"structuredErrors":true,"executionTranscriptContracts":[coven_client::transcript::CONTRACT]}),
                    ),
                ),
                (
                    "/api/v1/execution-transcript/read".into(),
                    200,
                    reply.to_string(),
                ),
            ],
        ...
        let mut client = DaemonClient::new(DaemonEndpoint::discover(&home.path).unwrap());
        let result = client.read_execution_transcript(&request);
        if field == "valid" {
            result.unwrap();
        } else {
            assert!(matches!(
                result,
                Err(ClientError::InvalidTranscriptResponse(_))
            ));
        }
        server.join().unwrap();
    }
